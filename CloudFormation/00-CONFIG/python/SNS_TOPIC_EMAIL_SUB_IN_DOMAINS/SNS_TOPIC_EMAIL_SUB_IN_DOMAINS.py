...
Description:
    Check whether the emails - subscribed to SNS topics - are from specified domains. The rule is NON_COMPLAINT if the email domains from a subscription with email or email-json protocols are not on the specified domains.
...
Rule Parameters:
    domainNames (Required)
      Comma-separated list of authorized domainNames (without the @, i.e. example.com,example2.com).

Scenarios:

    Scenario 1:
    Given: The Rule parameter domainNames is not provided or not valid
     Then: Return ERROR

    Scenario 2:
    Given: There is no SNS Topic in the AWS Account
     Then: Return NOT_APPLICABLE

    Scenario 3:
    Given: At least 1 SNS Topic is present in the AWS Account
      And: The parameter domainNames is configured and valid
      And: There is no email or email-json as protocol on any given SNS Topic
     Then: Return NOT_APPLICABLE on this SNS Topic

    Scenario 4:
    Given: At least 1 SNS Topic is present in the AWS Account
      And: The parameter domainNames is configured and valid
      And: At least one subscriber protocol is email or email-json
      And: At least one of the email address on the Endpoint is not from the specified domain
     Then: Return NON_COMPLAINT on this SNS Topic

    Scenario 5:
    Given: At least 1 SNS Topic is present in the AWS Account
      And: The parameter domainNames is configured and valid
      And: At least one subscriber protocol is email or email-json
      And: All email addresses on the Endpoint are from the specified domain
     Then: Return COMPLAINT on this SNS Topic
