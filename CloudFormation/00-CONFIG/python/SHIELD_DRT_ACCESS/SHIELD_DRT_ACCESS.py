...
Scenarios:
 Scenario: 1
  ...
  Scenario: 2
    ...
    And: Role for DRT access is not configured
    Then: Return NON_COMPLIANT with annotation "DRT team does not have access to account."
  Scenario: 3
    Given: AWS Shield Advanced is enabled
    And: Role for DRT access is configured
...
# Main Code #
#############
def drt_access_check(client):
    try:
        return client.describe_drt_access()
    except botocore.exceptions.ClientError as error:
        return error.response['Error']['Code']

def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    drt_access_enabled = drt_access_check(get_client('shield', event))
    if drt_access_enabled == 'ResourceNotFoundException':
        return build_evaluation(event['accountId'],
                                'NOT_APPLICABLE',
                                event,
                                annotation='AWS Shield Advanced is not enabled.')
    if 'RoleArn' in drt_access_enabled:
        return build_evaluation(event['accountId'],
                                'COMPLIANT',
                                event)
    return build_evaluation(event['accountId'],
