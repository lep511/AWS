...
Description:
    Checks whether the VPC Endpoint Service is configured for Manual Acceptance. The rule is NON_COMPLIANT if VPC Endpoint Service is configured to accept Endpoint requests automatically.
...
Scenarios:
  ...
  Scenario: 2
    ...
    And: "AcceptanceRequired" Key is set to False on DescribeVpcEndpoints.
    Then: Return NON_COMPLIANT

  Scenario: 3
    Given: At least one customer-owned VPC Endpoint Service is present.
    And: "AcceptanceRequired" Key is set to True on DescribeVpcEndpoints.
