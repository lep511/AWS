...
Rule Parameters:
  KmsKeyId
    (Optional) Comma-separated list of KMS keys ARN that should used to encrypt the Amazon Simple Notification Service
...
Scenarios:
  ...
  Scenario: 3
     Given: There is at least 1 SNS topic
      ...
      Then: Return NON_COMPLIANT with Annotation "The Amazon Simple Notification Service topic is not encrypted."
  Scenario: 4
     Given: There is at least 1 SNS topic
...
# Return all the topic arn in the account
def get_all_topic(sns_client):
    next_token = None
    result_list = []
