...
```bash
forecast-service$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
forecast-service$ sam logs -n HelloWorldFunction --stack-name forecast-service --tail
```
...
```bash
forecast-service$ pip install pytest pytest-mock --user
forecast-service$ python -m pytest tests/ -v
```
