...
def call_data_api(redshift_client, redshift_database, redshift_secret, redshift_workgroup, sql_statement, with_event=True):
    # execute the input SQL statement
    api_response = redshift_client.execute_statement(Database=redshift_database, SecretArn=redshift_secret
                                                    ,Sql=sql_statement, WorkgroupName=redshift_workgroup,WithEvent=True)
...
def lambda_handler(event, context):
    ...
    redshift_user = os.environ['redshift_secret']
    ...
    try:
        if action == "execute_report":
            ...
            api_response = call_data_api(redshift_client, redshift_database, redshift_user,  redshift_wg_id, sql_statement)
            return_status = 200
