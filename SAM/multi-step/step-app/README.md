...
```bash
step-app$ sam build --use-container
```
...
```bash
step-app$ sam logs -n StockCheckerFunction --stack-name step-app --tail
```
...
```bash
step-app$ pip install -r tests/requirements.txt --user
# unit test
step-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
step-app$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name step-app
```
